...
local_x = np.array([0])
# Image has 12788 rows and 40000 colums
...
c = np.array((rowsize, 40000), dtype=np.uint8)  # Part of image given to each process
comm.Bcast(c, root=0)
local_x = np.zeros(c, dtype=np.uint8)
...
comm.Reduce(star_count_node, total, op=MPI.SUM,
            root=0)  # Reduce to zero process
